...
import Image from "next/image"; // Optimizasyon için Next Image
        ...
        <main className="bg-gray-100 min-h-screen flex items-center justify-center p-6">
            <div className="flex flex-wrap justify-center gap-6 max-w-7xl">
                {mapCards.map((map) => (
                    <div
                        key={map.id}
                        className="group flex flex-col w-full sm:w-80 bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-500 ease-in-out hover:-translate-y-2 overflow-hidden border border-gray-100"
                    >
