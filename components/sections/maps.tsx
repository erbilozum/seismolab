...
import SeismicLoaderCard from "@/components/card/seismicLoaderCard";
...
const Maps = ({ selectedMapId }: MapsProps) => {
    ...
    useEffect(() => {
        const loadLeaflet = async () => {
            ...
            try {
                ...
                logger.error({ err }, "Failed to load Leaflet or fetch earthquake data");
            } finally {
                setIsLoading(false);
            }
        };
    ...
    return (
        <div className={"w-full h-full relative z-0"}>
            ...
            <MapContainer center={[37.6, 37.3]} zoom={6.8} zoomControl={false} className={"w-full h-full grayscale-15"}>
                ...
                {cities.map((city, index) => {
                    ...
                    const iconHtml = `
                        <div class="pulse-container" style="width: ${dynamicSize}px; height: ${dynamicSize}px;">
                            ...
                            <div class="pulse-dot"></div>                      </div>`;
                    const customIcon = L.divIcon({className: "custom-leaflet-icon", html: iconHtml, iconSize: [dynamicSize, dynamicSize], iconAnchor: [dynamicSize / 2, dynamicSize / 2],});
                    return (
                        <Marker key={`${city.name}-${index}`} position={city.coords} icon={customIcon}>
                            <Popup className={"minimal-popup"}>
                                <ToolTipCard data={city} />
                            </Popup>
                        </Marker>
                    );
                })}
