...
const ToolTipCard = ({ data }: TooltipCardProps) => {
    ...
    return (
        <motion.div
            initial={{ opacity: 0, scale: 0.95, y: 5 }} animate={{ opacity: 1, scale: 1, y: 0 }} className={"w-64 bg-white/95 backdrop-blur-xl rounded-2xl overflow-hidden shadow-[0_25px_50px_-12px_rgba(0,0,0,0.15)] border border-slate-200/60 select-none font-sans"}>
            {/** HEADER */}
            ...
            <div className={"p-4 space-y-3"}>
                ...
                <div className={"grid grid-cols-2 gap-2 pt-1.5"}>
                    ...
                    {/** DEATH */}
                    {data.deaths !== undefined && (<div className={"flex flex-col items-center bg-slate-50/50 rounded-xl p-2.5 border border-slate-100 text-center transition-colors hover:bg-white"}>
                            <p className={"text-[9px] text-slate-400 font-bold uppercase mb-1 leading-tight"}>
                                {t("marker.deaths")}
                            </p>
                    ...
                    {/** INJURED PEOPLE */}
                    {data.injured !== undefined && (<div className={"flex flex-col items-center bg-slate-50/50 rounded-xl p-2.5 border border-slate-100 text-center transition-colors hover:bg-white"}>
                            <p className={"text-[9px] text-slate-400 font-bold uppercase mb-1 leading-tight"}>{t("marker.injured")}</p>
                            <p className={"text-sm font-black text-yellow-600"}>{data.injured}</p>
                        </div>)}
                </div>
            </div>
            {/** FOOTER */}
            <div className={"h-1.5 w-full bg-linear-to-r from-transparent via-slate-200 to-transparent mb-1 opacity-40"} />
        </motion.div>
    );
};
